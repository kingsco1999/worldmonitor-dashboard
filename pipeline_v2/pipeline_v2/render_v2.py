...
const TV_SOURCES = [
  {id:'bloomberg', name:'BLOOMBERG', zh:'彭博財經',
   hls:'https://ciner.daioncdn.net/bloomberght/bloomberght.m3u8',
  ...
  {id:'sky', name:'SKY NEWS', zh:'天空新聞',
   hls:'https://skynews-skynewsuk-1-eu.rakuten.wurl.tv/playlist.m3u8',
...
function initTV(){
  ...
  function play(){
    ...
    // 優先 HLS 直播，失敗再 fallback 去 YouTube
    if(s.hls){
      loadHls(s);
      // HLS 失敗 fallback：等 8 秒冇畫面就試 YouTube
      loadT=setTimeout(()=>{
        if(playing && hlsP===null){
          // HLS 初始化失敗，fallback YouTube
          if(s.embed){
